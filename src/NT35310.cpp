...
NT35310::NT35310(spi_device_num_t spiDev, spi_chip_select_t spiCS, uint8_t RSTNum, uint8_t DCNum, uint8_t WRNum, uint16_t width, uint16_t height) {
    this->spiDev = spiDev;
    this->spiCS  = spiCS;
...
void NT35310::fillArea(uint32_t color, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
    ...
    //this->fillDMA((color & 0xFFFF) | (color << 16), 32, ((x2 + 1) - x1) * ((y2 + 1) - y1));
#endif
